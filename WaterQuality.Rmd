...
spaceSel=c("oligotrophic","mesotrophic","aquatic","irrigation","recreational","drink","eutrophic")
...
plot(vegan::scores(pca,display = "species"),type="n",xlim=range(sp[,1]*1.1),ylab="PC2",xlab="PC1")
...
m=1
sp=vegan::scores(pca,display = "species",choices=axes*m)
plot(vegan::scores(pca,display = "species"),type="n",xlim=range(sp[,1]*1.1),ylab="PC3",xlab="PC2")
text(sp,labels=rownames(sp),col="grey35")
arrows(0,0,sp[,1]*0.85,sp[,2]*0.85,length=0.07)
abline(h=0);abline(v=0)
...
limComp=round(limFreqTable/limTotTable,2)
